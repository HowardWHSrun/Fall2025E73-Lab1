// JavaScript build of plot.ts (no build step required). Exposes window.QMPlot.
(function(){
  const E_CHARGE = 1.602176634e-19; // J/eV
  const NM = 1e-9;

  function ensureEl(container){
  ...
  function renderPotentialAndStatesSVG(container, x, V, energiesJ, psi, title, opts){
    opts = opts || {};
    ...
    const line = (x1,y1,x2,y2,color,width,dash)=>{
      ...
      l.setAttribute('stroke', color); if (width) l.setAttribute('stroke-width', String(width));
      if (dash) l.setAttribute('stroke-dasharray', dash);
      svg.appendChild(l);
    };
    const path = (d, stroke, width, fill, opacity, dash)=>{
    ...
    const text = (x,y,s,size=12,color='#111827',anchor='start')=>{
      const t = document.createElementNS(svgns, 'text');
      t.setAttribute('x', String(x)); t.setAttribute('y', String(y));
      t.setAttribute('fill', color); t.setAttribute('font-size', String(size));
      t.setAttribute('font-family', 'Arial, sans-serif'); t.setAttribute('text-anchor', anchor);
    ...
    let ymin = Math.min(Vmin, Emin), ymax = Math.max(Vmax, Emax);
    ...
    for (let j = 0; j < psi[0].length; j++) {
      for (let i = 0; i < x.length; i++) { const v = psi[i][j]; maxProb = Math.max(maxProb, v*v); maxAmp = Math.max(maxAmp, Math.abs(v)); }
    }
    ...
    for (let j = 0; j < psi[0].length; j++) {
      ...
      if (showProb) {
        ...
        for (let i = 0; i < x.length; i++) {
          const prob = psi[i][j] * psi[i][j] * probScale; const px = xToPx(x_nm[i]); const py = yToPx(Ej + prob);
          d += (i === 0 ? `M ${px} ${py}` : ` L ${px} ${py}`);
        }
        path(d, color, 2.5);
  ...
  function renderProbabilityDensitySVG(container, x, energiesJ, psi, title){
